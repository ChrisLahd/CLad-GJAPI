...
Forked from tenta, revamped by ChrisLad
...
import GameJoltAPISupport as Api;
...
class GameJoltAPI // Connects to tentools.api.FlxGameJolt
{
    ...
    public static var totalTrophies:Float = Api.TROPHIES_ACHIEVED + Api.TROPHIES_MISSING;
    ...
    {
        if(username)return Api.username;
        else return Api.usertoken;
    }
    ...
    public static function connect() /* Sets the game ID and game key */
    {
        ...
        Api.init(Std.int(GJKeys.id), Std.string(GJKeys.key), false);
    }
    ...
    public static function getTrophy(trophyID:Int) /* Awards a trophy to the user! */
    {
        if(userLogin)
        {
            Api.addTrophy(trophyID, function(){trace("Unlocked a trophy with an ID of "+trophyID);});
        }
    }
    ...
    // {
    //     var value:Bool;
    //     Api.fetchTrophy(id, function(data:Map<String, String>)
    //         {
    ...
    //         {Api.fetchTrophy(0, function(trophy:Map<String, String>)
    //             {
    ...
    //     Api.addScore(Std.string(score + " score - Achived on " + Date.now()), score, tableID);
    // }
    // public static function getProfileImage():BitmapData /* Gets the user's profile picture */
    // {
    //     var image;
    //     Api.fetchAvatarImage(function(data:BitmapData){image = data;});
    //     return image;
    // }
    public static function startSession() /*Starts the session */
    {
        Api.openSession(function()
            {
    ...
    {
        Api.pingSession(true, function(){trace("Ping!");});
    }
    public static function closeSession() /* Closes the session, used for signing out */
    {
        Api.closeSession(function(){trace('Closed out the session');});
    }
